...
ICACHE_FLASH_ATTR u32_t SPIFFS_buffer_bytes_for_filedescs(spiffs *fs, u32_t num_descs) {
  return num_descs * sizeof(spiffs_fd);
}
#if SPIFFS_CACHE
ICACHE_FLASH_ATTR u32_t SPIFFS_buffer_bytes_for_cache(spiffs *fs, u32_t num_pages) {
...
ICACHE_FLASH_ATTR s32_t SPIFFS_probe_fs(spiffs_config *config) {
...
ICACHE_FLASH_ATTR s32_t SPIFFS_creat(spiffs *fs, const char *path, spiffs_mode mode) {
#if SPIFFS_READ_ONLY
...
ICACHE_FLASH_ATTR spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode) {
  (void)mode;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  if (strlen(path) > SPIFFS_OBJ_NAME_LEN - 1) {
...
ICACHE_FLASH_ATTR spiffs_file SPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
#if !SPIFFS_READ_ONLY
ICACHE_FLASH_ATTR static s32_t spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf, u32_t offset, s32_t len) {
  (void)fs;
  s32_t res = SPIFFS_OK;
  s32_t remaining = len;
...
ICACHE_FLASH_ATTR s32_t SPIFFS_remove(spiffs *fs, const char *path) {
#if SPIFFS_READ_ONLY
  (void)fs; (void)path;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
...
ICACHE_FLASH_ATTR static s32_t spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s) {
  (void)fh;
...
ICACHE_FLASH_ATTR s32_t SPIFFS_stat(spiffs *fs, const char *path, spiffs_stat *s) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  if (strlen(path) > SPIFFS_OBJ_NAME_LEN - 1) {
    SPIFFS_API_CHECK_RES(fs, SPIFFS_ERR_NAME_TOO_LONG);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_close(spiffs *fs, spiffs_file fh) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_rename(spiffs *fs, const char *old_path, const char *new_path) {
#if SPIFFS_READ_ONLY
...
ICACHE_FLASH_ATTR spiffs_DIR *SPIFFS_opendir(spiffs *fs, const char *name, spiffs_DIR *d) {
  (void)name;
...
ICACHE_FLASH_ATTR static s32_t spiffs_read_dir_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
ICACHE_FLASH_ATTR s32_t SPIFFS_eof(spiffs *fs, spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_tell(spiffs *fs, spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_set_file_callback_func(spiffs *fs, spiffs_file_callback cb_func) {
  SPIFFS_LOCK(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_ix_map(spiffs *fs,  spiffs_file fh, spiffs_ix_map *map,
...
ICACHE_FLASH_ATTR s32_t SPIFFS_ix_unmap(spiffs *fs,  spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_ix_remap(spiffs *fs, spiffs_file fh, u32_t offset) {
  s32_t res = SPIFFS_OK;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_bytes_to_ix_map_entries(spiffs *fs, u32_t bytes) {
  SPIFFS_API_CHECK_CFG(fs);
...
ICACHE_FLASH_ATTR s32_t SPIFFS_ix_map_entries_to_bytes(spiffs *fs, u32_t map_page_ix_entries) {
  SPIFFS_API_CHECK_CFG(fs);
