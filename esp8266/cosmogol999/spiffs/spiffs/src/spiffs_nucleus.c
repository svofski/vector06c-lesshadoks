...
ICACHE_FLASH_ATTR static s32_t spiffs_page_data_check(spiffs *fs, spiffs_fd *fd, spiffs_page_ix pix, spiffs_span_ix spix) {
  s32_t res = SPIFFS_OK;
...
#if !SPIFFS_READ_ONLY
ICACHE_FLASH_ATTR static s32_t spiffs_page_index_check(spiffs *fs, spiffs_fd *fd, spiffs_page_ix pix, spiffs_span_ix spix) {
  s32_t res = SPIFFS_OK;
  if (pix == (spiffs_page_ix)-1) {
    // referring to page 0xffff...., bad object index
...
ICACHE_FLASH_ATTR s32_t spiffs_probe(
    spiffs_config *cfg) {
...
ICACHE_FLASH_ATTR static s32_t spiffs_obj_lu_scan_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
ICACHE_FLASH_ATTR s32_t spiffs_obj_lu_scan(
  ...
  while (bix < fs->block_count) {
    ...
    if (magic != SPIFFS_MAGIC(fs, bix)) {
      ...
      if (unerased_bix == (spiffs_block_ix)-1) {
        ...
        unerased_bix = bix;
        //printf("[unerased_bix<-%d]", unerased_bix);
      } else {
        // more than one unerased block, bail out
        //printf("[more than one unerased block]");
    ...
    //printf("[erase_count=%d]", erase_count);
    SPIFFS_CHECK_RES(res);
    //printf("[continue]");
  ...
  //printf("[out of while bix < fs->block_count bix=%d]", bix);
...
ICACHE_FLASH_ATTR static s32_t spiffs_obj_lu_find_id_and_span_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
ICACHE_FLASH_ATTR static void spiffs_update_ix_map(spiffs *fs,
...
ICACHE_FLASH_ATTR static s32_t spiffs_populate_ix_map_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
ICACHE_FLASH_ATTR s32_t spiffs_populate_ix_map(spiffs *fs, spiffs_fd *fd, u32_t vec_entry_start, u32_t vec_entry_end) {
  s32_t res;
  spiffs_ix_map *map = fd->ix_map;
...
ICACHE_FLASH_ATTR static s32_t spiffs_object_find_object_index_header_by_name_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
ICACHE_FLASH_ATTR static s32_t spiffs_obj_lu_find_free_obj_id_bitmap_v(spiffs *fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
...
ICACHE_FLASH_ATTR static s32_t spiffs_obj_lu_find_free_obj_id_compact_v(spiffs *fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
    const void *user_const_p, void *user_var_p) {
  (void)user_var_p;
...
ICACHE_FLASH_ATTR s32_t spiffs_obj_lu_find_free_obj_id(spiffs *fs, spiffs_obj_id *obj_id, const u8_t *conflicting_name) {
  s32_t res = SPIFFS_OK;
...
#if SPIFFS_TEMPORAL_FD_CACHE
// djb2 hash
ICACHE_FLASH_ATTR static u32_t spiffs_hash(spiffs *fs, const u8_t *name) {
  (void)fs;
  u32_t hash = 5381;
  u8_t c;
  int i = 0;
...
ICACHE_FLASH_ATTR s32_t spiffs_fd_find_new(spiffs *fs, spiffs_fd **fd, const char *name) {
#if SPIFFS_TEMPORAL_FD_CACHE
  u32_t i;
...
#if SPIFFS_TEMPORAL_FD_CACHE
ICACHE_FLASH_ATTR void spiffs_fd_temporal_cache_rehash(
    spiffs *fs,
    const char *old_path,
    const char *new_path) {
  u32_t i;
