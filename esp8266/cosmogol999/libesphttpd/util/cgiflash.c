			...
			if (def->type==CGIFLASH_TYPE_FW && memcmp(data, "EHUG", 4)==0) {
				...
				if (state->state!=FLST_ERROR) {
					...
					if (system_upgrade_userbin_check()==1) {
						...
						httpd_printf("FUCK! Flashing user2.bin from ota image\n");
						state->len=h->len2;
						state->skip=h->len1;
				...
				httpd_printf("Did not recognize flash image type!\n");
                        ...
                        httpd_printf("FLST_SKIP: state->skip=%d dataLen=%d: ", state->skip, dataLen);
			//Skip bytes without doing anything with them
			if (state->skip>dataLen) {
				//Skip entire buffer
				state->skip-=dataLen;
				dataLen=0;
                                httpd_printf(" skipping entire buffer\n");
			} else {
				...
				if (state->len) state->state=FLST_WRITE; else state->state=FLST_DONE;
                                httpd_printf(" skipping part buffer, next state=%d\n", state->state);
			}
		} else if (state->state==FLST_WRITE) {
                        httpd_printf("State FLST_WRITE\n");
