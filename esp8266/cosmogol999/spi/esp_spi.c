...
ICACHE_FLASH_ATTR void spi_get_settings(uint8_t bus, spi_settings_t *s)
{
...
ICACHE_FLASH_ATTR inline static void _set_size(uint8_t bus, uint8_t bytes)
{
...
ICACHE_FLASH_ATTR inline static void _wait(uint8_t bus)
{
    while (SPI(bus).CMD & SPI_CMD_USR)
        ;
}

ICACHE_FLASH_ATTR inline static void _start(uint8_t bus)
{
    SPI(bus).CMD |= SPI_CMD_USR;
}

ICACHE_FLASH_ATTR inline static void _store_data(uint8_t bus, const void *data, size_t len)
{
    uint8_t words = len / 4;
    uint8_t tail = len % 4;
...
ICACHE_FLASH_ATTR inline static uint32_t _swap_bytes(uint32_t value)
{
    return (value << 24) | ((value << 8) & 0x00ff0000) | ((value >> 8) & 0x0000ff00) | (value >> 24);
}

ICACHE_FLASH_ATTR inline static uint32_t _swap_words(uint32_t value)
{
    return (value << 16) | (value >> 16);
}

ICACHE_FLASH_ATTR static void _spi_buf_prepare(uint8_t bus, size_t len, spi_endianness_t e, spi_word_size_t word_size)
{
...
ICACHE_FLASH_ATTR static void _spi_buf_transfer(uint8_t bus, const void *out_data, void *in_data,
...
ICACHE_FLASH_ATTR uint8_t spi_transfer_8(uint8_t bus, uint8_t data)
{
    uint8_t res;
...
ICACHE_FLASH_ATTR uint16_t spi_transfer_16(uint8_t bus, uint16_t data)
{
    uint16_t res;
...
ICACHE_FLASH_ATTR uint32_t spi_transfer_32(uint8_t bus, uint32_t data)
{
    uint32_t res;
...
ICACHE_FLASH_ATTR static void _rearm_extras_bit(uint8_t bus, bool arm)
{
...
ICACHE_FLASH_ATTR size_t spi_transfer(uint8_t bus, const void *out_data, void *in_data, size_t len, spi_word_size_t word_size)
{
...
ICACHE_FLASH_ATTR static void _spi_buf_read(uint8_t bus, uint8_t b, void *in_data,
    size_t len, spi_endianness_t e, spi_word_size_t word_size)
{
    _wait(bus);
    size_t bytes = len * (uint8_t)word_size;
...
ICACHE_FLASH_ATTR void spi_read(uint8_t bus, uint8_t out_byte, void *in_data, size_t len, spi_word_size_t word_size)
{
    spi_endianness_t e = spi_get_endianness(bus);
    uint8_t buf_size = _SPI_BUF_SIZE / (uint8_t)word_size;
...
ICACHE_FLASH_ATTR static void _repeat_send(uint8_t bus, uint32_t *dword, int32_t *repeats,
...
ICACHE_FLASH_ATTR void spi_repeat_send_8(uint8_t bus, uint8_t data, int32_t repeats)
{
    uint32_t dword = data << 24 | data << 16 | data << 8 | data;
    _repeat_send(bus, &dword, &repeats, SPI_8BIT);
}

ICACHE_FLASH_ATTR void spi_repeat_send_16(uint8_t bus, uint16_t data, int32_t repeats)
{
    uint32_t dword = data << 16 | data;
    _repeat_send(bus, &dword, &repeats, SPI_16BIT);
}

ICACHE_FLASH_ATTR void spi_repeat_send_32(uint8_t bus, uint32_t data, int32_t repeats)
{
