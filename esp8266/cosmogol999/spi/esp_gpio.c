...
ICACHE_FLASH_ATTR void gpio_enable(const uint8_t gpio_num, const gpio_direction_t direction)
{
    if (gpio_num == 16) {
...
ICACHE_FLASH_ATTR void __attribute__((weak)) IRAM gpio_interrupt_handler(void *arg)
{
...
ICACHE_FLASH_ATTR void gpio_set_interrupt(const uint8_t gpio_num, const gpio_inttype_t int_type, gpio_interrupt_handler_t handler)
{
